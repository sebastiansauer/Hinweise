...
2.  Bei Aufgaben zur "*Bayes-Box*" (Erstellung einer Gitterwert-Tabelle) gelten folgende Maßgaben:
  -  Handelt es sich um Parameter mit einem begrenzten Wertebereich (wie etwa *Anteile*), so ist der ganze Wertebereich zu modellieren. Es sind *100* verschiedene Parameterwerte zu berechnen (wenn möglich, gleichabständig verteilt).
...
2.  Der *Toleranzbereich* einer Lösung liegt bei 5% des Wertes der Musterlösung (sofern nicht anders bei der jeweiligen Aufgabe angegeben). Toleranzbereiche werden verwendet für Aufgaben mit Zufallszahlen: Lösungen solcher Aufgaben können schwanken. Wenn Sie allerdings den angegebenen Seed-Wert gesetzt haben, so sollte Ihre (korrekte) Lösung exakt der Musterlösung entsprechen. Beispiele: 
  - Der Wert laut Musterlösung liegt bei 100; dann erstreckt sich der Toleranzbereich von 95 bis 105. 
  - Der Wert laut Musterlösung liegt bei .05; dann erstreckt sich der Toleranzbereich um ±0.0025 Punkte um 0.05.
...
Die folgende Liste zeigt, welche Datensätze in der Prüfung verwendet werden.
...
d1 <- read.csv("https://raw.githubusercontent.com/sebastiansauer/teachertools/main/data/teaching_df.csv") |> 
  rename(Shortname = datasets_names,
         CSV = source,
         Doc = doc)
...
d2 <- read.csv("https://raw.githubusercontent.com/sebastiansauer/teachertools/main/data/datasets-openintro.csv") |> 
  rename(Shortname = Item)

d_joint <-
  d1 |>  
  bind_rows(d2) |> 
  mutate(id = row_number()) |> 
  select(id, Shortname, Package, CSV, Doc)
```
